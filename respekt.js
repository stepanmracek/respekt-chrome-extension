...
respektApp.factory('issueDownloader', function() {
    var issueDownloader = {
        getContent: function(callback) {
            ...
            var url = 'http://www.respekt.cz/tydenik/2015/33';
            var x = new XMLHttpRequest();
            x.open('GET', url);
            ...
            x.onload = function() {
                if (x.status == 200) {
                    ...
                    for (var i = 0; i < section.length; i++) {
                        ...
                        resultData.categories.push(section[i].innerHTML);
                    }

                    var items = doc.getElementsByClassName('issuedetail-categorized-item');
...
respektApp.controller('issueCtrl', function($scope, issueDownloader) {
    ...
    issueDownloader.getContent(function(data) {
        $scope.issue = data;
        $scope.$apply();
    });
    ...
    x.onload = function() {
      if (x.status == 200) {
        ...
        for (var i = 0; i < section.length; i++) {
          ...
          var items = section[i].getElementsByClassName('issuedetail-categorized-item');
          for (var j = 0; j < items.length; j++) {
            console.log("  " + items[j].innerHTML);
          }
        }
